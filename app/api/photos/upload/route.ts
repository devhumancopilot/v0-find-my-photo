...
export async function POST(request: NextRequest) {
  try {
    ...
    // Trigger N8N webhook for manual image upload processing
    // n8n will handle ALL storage operations (Supabase Storage, Database inserts, etc.)
    if (n8nPayloads.length > 0) {
      try {
        const webhookResult = await triggerWebhook(process.env.N8N_WEBHOOK_MANUAL_IMAGE_UPLOAD!, {
          user_id: user.id,
          images: n8nPayloads,
